/*
api

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package UpStudyGoSdk

import (
	"encoding/json"
)

// checks if the V1ShowStepsPost200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &V1ShowStepsPost200Response{}

// V1ShowStepsPost200Response struct for V1ShowStepsPost200Response
type V1ShowStepsPost200Response struct {
	Data *SolverSelfFullResponseText `json:"data,omitempty"`
	ErrMsg *string `json:"err_msg,omitempty"`
}

// NewV1ShowStepsPost200Response instantiates a new V1ShowStepsPost200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewV1ShowStepsPost200Response() *V1ShowStepsPost200Response {
	this := V1ShowStepsPost200Response{}
	return &this
}

// NewV1ShowStepsPost200ResponseWithDefaults instantiates a new V1ShowStepsPost200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewV1ShowStepsPost200ResponseWithDefaults() *V1ShowStepsPost200Response {
	this := V1ShowStepsPost200Response{}
	return &this
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *V1ShowStepsPost200Response) GetData() SolverSelfFullResponseText {
	if o == nil || IsNil(o.Data) {
		var ret SolverSelfFullResponseText
		return ret
	}
	return *o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ShowStepsPost200Response) GetDataOk() (*SolverSelfFullResponseText, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *V1ShowStepsPost200Response) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given SolverSelfFullResponseText and assigns it to the Data field.
func (o *V1ShowStepsPost200Response) SetData(v SolverSelfFullResponseText) {
	o.Data = &v
}

// GetErrMsg returns the ErrMsg field value if set, zero value otherwise.
func (o *V1ShowStepsPost200Response) GetErrMsg() string {
	if o == nil || IsNil(o.ErrMsg) {
		var ret string
		return ret
	}
	return *o.ErrMsg
}

// GetErrMsgOk returns a tuple with the ErrMsg field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *V1ShowStepsPost200Response) GetErrMsgOk() (*string, bool) {
	if o == nil || IsNil(o.ErrMsg) {
		return nil, false
	}
	return o.ErrMsg, true
}

// HasErrMsg returns a boolean if a field has been set.
func (o *V1ShowStepsPost200Response) HasErrMsg() bool {
	if o != nil && !IsNil(o.ErrMsg) {
		return true
	}

	return false
}

// SetErrMsg gets a reference to the given string and assigns it to the ErrMsg field.
func (o *V1ShowStepsPost200Response) SetErrMsg(v string) {
	o.ErrMsg = &v
}

func (o V1ShowStepsPost200Response) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o V1ShowStepsPost200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	if !IsNil(o.ErrMsg) {
		toSerialize["err_msg"] = o.ErrMsg
	}
	return toSerialize, nil
}

type NullableV1ShowStepsPost200Response struct {
	value *V1ShowStepsPost200Response
	isSet bool
}

func (v NullableV1ShowStepsPost200Response) Get() *V1ShowStepsPost200Response {
	return v.value
}

func (v *NullableV1ShowStepsPost200Response) Set(val *V1ShowStepsPost200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableV1ShowStepsPost200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableV1ShowStepsPost200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableV1ShowStepsPost200Response(val *V1ShowStepsPost200Response) *NullableV1ShowStepsPost200Response {
	return &NullableV1ShowStepsPost200Response{value: val, isSet: true}
}

func (v NullableV1ShowStepsPost200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableV1ShowStepsPost200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


